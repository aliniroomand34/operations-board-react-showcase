...
[60-second demo script](./RECRUITER_DEMO_SCRIPT.md). Scope stays clear: what this
...
> The board had the highest reviewer signal for the least privacy risk: multi-column
> workflow, assign / in-progress / complete, drag/drop plus a keyboard path, and a
> clean UI / Logic / API / helpers split. Broader private surfaces would force either
> oversharing domain language or a demo that looks incomplete. One coherent slice is
> easier to defend in 30 seconds.
...
> extensions. Those explain why the board existed. This public repo ships the
> anonymized board over a mock API.
...
> renamed (operation request, client, inventory batch) and CSS chrome uses `ops-*`
> naming. What left was sensitive product language — not polish, density, or
> workflow clarity.
...
| --- | --- |
| Frontend orchestration for queue → assign → complete | Live Redis / WebSocket engineering in this repo |
| Pure, testable domain helpers | Ownership of the full private backend |
| Typed contracts across UI, hook, API, mocks | Exhaustive TypeScript mastery across every codebase |
...
| “This demo runs a 30-source pool and live team RBAC.” | “Those were private capacity signals; the public demo documents them and ships the board UI only.” |
| “This is the full private product.” | “This is a safe frontend projection of one operations board.” |
...
- [PUBLIC_SURFACE.md](./PUBLIC_SURFACE.md) — publishable path boundary
